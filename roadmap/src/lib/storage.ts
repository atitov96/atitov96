...
export function saveProgress(userId: number, itemId: string, completed: boolean) {
  try {
    const storageData = localStorage.getItem(STORAGE_KEY);
    const progress: Record<number, StorageProgress> = storageData ? JSON.parse(storageData) : {};
